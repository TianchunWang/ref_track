# 11-parameter fitting using lmfit with PARAMETER STANDARDIZATION
...
def lmfit_objective_std(params_scaled):
    """Objective function with standardized parameters"""
    params_original = {}
...
print("="*70)
print(f"{'Parameter':<20s} {'Original':<12s} {'Scaled':<12s} {'Range':<20s}")
print("-"*70)

for name in param_bounds.keys():
    orig_val = initial_values[name]
    scaled_val = params_scaled[name].value
    orig_range = f"[{param_bounds[name][0]:.2f}, {param_bounds[name][1]:.2f}]"
    print(f"{name:<20s} {orig_val:<12.4f} {scaled_val:<12.4f} {orig_range:<20s}")
print("="*70)

print("\n" + "="*70)
print("FITTING CONFIGURATION")
print("="*70)
print(f"Total parameters: 11")
...
fig.add_trace(go.Scatter(
    ...
    mode='lines', 
    name='Optimized Fit (11-param, standardized)', 
...
title_text = '11-Parameter Fitting with Standardization'
fig.update_layout(
...
results_dict = {
    'n_parameters': 11,
    'optimization_method': 'bfgs_standardized',
...
output_filename = 'fitting_results_11param_standardized.csv'
results_df.to_csv(output_filename, index=False)
print(f"\n[OK] Results exported to: {output_filename}")
print(f"[OK] Total columns exported: {len(results_dict)}")
print(f"\nKey information:")
print(f"  - Method: BFGS with standardized parameters")
